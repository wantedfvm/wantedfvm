...
ROWS = [
    ...
    ("kv", "Focus", "Fullstack & Scripting"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Frontend", "HTML, CSS, Tailwind, JS, TypeScript, Next.js"),
    ("kv", "Backend", "Node.js, PHP, Python, C++, C#"),
    ("kv", "Databases", "MySQL, PostgreSQL"),
    ("kv", "Other", "Electron, Lua"),
    ("gap",),
    ("sec", "Highlights"),
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">wantedfvm@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">wantedfvm</tspan><tspan fill="{MUTED}">@</tspan>'
